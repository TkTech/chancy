...
@pytest.mark.asyncio
# @pytest.mark.parametrize("chancy", [{"notifications": False}], indirect=True)
async def test_regression_48(chancy: Chancy, worker: Worker):
    ...
    """
    await chancy.declare(Queue("single", concurrency=1, polling_interval=5, executor=Chancy.Executor.Async))
